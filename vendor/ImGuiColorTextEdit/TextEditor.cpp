...
void TextEditor::SetLanguageDefinition(LanguageDefinitionId aValue)
{
    ...
    switch (mLanguageDefinitionId)
    {
        ...
        case LanguageDefinitionId::Asm:
            ...
            break;
        case LanguageDefinitionId::AsmArm:
            mLanguageDefinition = &(LanguageDefinition::AsmArm());
            break;
    }
...
bool TextEditor::Render(const char* aTitle, bool aParentIsFocused, const ImVec2& aSize, bool aBorder)
{
    ...
    {
        if (ImGui::MenuItem("Copy", nullptr, false, AnyCursorHasSelection())) // Enable only if there is a selection
        {
            Copy();
        }
        if (ImGui::MenuItem("Paste", nullptr, false, !mReadOnly && ImGui::GetClipboardText() != nullptr)) // Enable only if not read-only and clipboard has text
        {
            Paste();
        }
        ImGui::EndPopup();
...
bool TextEditor::FindMatchingBracket(int aLine, int aCharIndex, Coordinates& out)
{
    ...
    {
        char shouldCloseChar = mLines[aLine][aCharIndex].mChar;
        char openChar = CLOSE_TO_OPEN_CHAR.at(shouldCloseChar);
        while (Move(currentLine, currentCharIndex, true))
        {
            if (currentCharIndex < mLines[currentLine].size())
            {
                ...
                else if (currentChar == shouldCloseChar)
                    counter++;
            }
        }
    }
    else if (OPEN_TO_CLOSE_CHAR.find(mLines[aLine][aCharIndex].mChar) != OPEN_TO_CLOSE_CHAR.end())
    {
        ...
        char shouldCloseChar = OPEN_TO_CLOSE_CHAR.at(openChar);
        while (Move(currentLine, currentCharIndex))
        {
            if (currentCharIndex < mLines[currentLine].size())
            {
                char currentChar = mLines[currentLine][currentCharIndex].mChar;
                if (currentChar == shouldCloseChar)
                {
                    counter--;
                    if (counter == 0)
                    {
...
void TextEditor::HighlightDebugCurrentLine(int line) {
    mLineToHighlight = line;
}

void TextEditor::HighlightBreakpoints(int line, bool ignoreHighlight) {
    if (line == -1){
        mLinesToHighlight.clear();
    }

    if (!ignoreHighlight){
        mLinesToHighlight.push_back(line);
    }
}
    ...
    if (!mLines.empty())
    {
        ...
        for (int lineNo = mFirstVisibleLine; lineNo <= mLastVisibleLine && lineNo < mLines.size(); lineNo++)
        {
            ...
            for (int c = 0; c <= mState.mCurrentCursor; c++)
            {
                ...
                if (std::find(mLinesToHighlight.begin(), mLinesToHighlight.end(),lineNo) != mLinesToHighlight.end()) {
                    ImVec2 lineEndScreenPos = ImVec2(lineStartScreenPos.x + mContentWidth, lineStartScreenPos.y + mCharAdvance.y);
                    drawList->AddRectFilled(lineStartScreenPos, lineEndScreenPos, mPalette[(int)PaletteIndex::DebugBreakpointLine]);
                }

                if (lineNo == mLineToHighlight) {
                    ImVec2 lineEndScreenPos = ImVec2(lineStartScreenPos.x + mContentWidth, lineStartScreenPos.y + mCharAdvance.y);
                    drawList->AddRectFilled(lineStartScreenPos, lineEndScreenPos, mPalette[(int)PaletteIndex::DebugCurrentLine]);
                }
